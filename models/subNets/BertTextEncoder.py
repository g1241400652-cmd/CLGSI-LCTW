...
class BertTextEncoder(nn.Module):
    def __init__(self, language='en', use_finetune=False):
        ...
        # directory is fine
        pretrained_model_en = os.environ.get('CLGSI_BERT_BASE_UNCASED_SNAPSHOT')
        if language == 'en' and not pretrained_model_en:
            raise RuntimeError('CLGSI_BERT_BASE_UNCASED_SNAPSHOT is required')
        pretrained_model_cn = 'bert-base-chinese'
        if language == 'en':                 #虽然但是，这里并没有用到tokenizer，因为原始数据里面已经使用的bert
            self.tokenizer = tokenizer_class.from_pretrained(
                pretrained_model_en, do_lower_case=True, local_files_only=True
            )
            self.model = model_class.from_pretrained(
                pretrained_model_en, local_files_only=True
            )
        elif language == 'cn':
